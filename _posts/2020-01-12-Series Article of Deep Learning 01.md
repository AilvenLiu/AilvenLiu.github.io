...
4. `args`。模块的具体参数，如第五层卷积`Conv`模块的`[512， 3， 2]`，当`width_multiple = 0.5`时，表示“输入通道为128（根据上一层），输出通道为256，卷积核尺寸3 $\times$ 3，步长为2”的卷积模块。 **这里需要格外注意一点，args 不需要指定输入通道数 `c_in`，yolov5会自动将该参数解析为上一层的输出。解析文件在哪儿没找到。**          
