...
TCP 每发送一条报文，都要等到对方返回 ACK 才能发送下一条，效率太低。为解决这一难题，TCP 引入了**窗口**的概念，窗口内的数据无需等待确认应答即可发送。这里引出一条知识点：TCP 发送端的窗口大小取决于接收方的窗口大小，接收方窗口大小是其窗口控制参数和拥塞窗口大小共同决定的，是两者中的小值；同时由于TCP 是全双工通信，通信两端各自维护一个自己的接收窗口。        
