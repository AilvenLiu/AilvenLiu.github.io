...
1. DNS 解析。搜索顺序是浏览器自身DNS 缓存、操作系统 DNS 缓存、本地 Host 文件，和本地 DNS 服务器。对于向本地 DNS 服务器发起查询，如果要查询的域名包含在本地配置区域资源中，则返回结果给用户机，完成域名解析；否则向上级 DNS 服务器发起迭代或递归查询。最终完成 域名向 IP 的解析。             
2. TCP 连接。浏览器获取到 IP 地址后，向服务器请求建立连接，发起三次握手。       
...
**Http 1.0 到 Http 1.1**          
1. Http 1.1 提出了长连接，可以在一个 TCP 中不间断的发送请求。           
...
4. Http 2.0 以二进制流而非明文传输数据，无结构的二进制帧流数据占用更小。             
5. Http 2.0 是支持 Https 传输的，由于其在 Http 层和 TCP 层之间加了一个 SSL 层。    
