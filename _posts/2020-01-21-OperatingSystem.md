...
**遗留问题：线程也好，进程也好，切换的时候他们的上下文信息保存到那里去了？又从哪里恢复？进程的页表保存在哪里、又从哪里恢复？**       

特别的，对于 Linux 而言，进程和线程没有严格的区分，其上下文信息都保存在 task_struct 结构体中，只是线程没有独立的虚拟内存，也即在 task_struct 中的 mm_struct 这一存储虚拟内存信息的结构体与进程内其他线程共享。          
