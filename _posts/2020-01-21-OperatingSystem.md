...
分也就是把整个虚拟和物理内存空间切成一段段连续且固定尺寸的小空间，称为 **页** 。Linux 下，每一页的大小为 4KB 。虚拟地址和物理地址之间通过页表来映射。每个进程有一张页表，虚拟每一页虚拟内存空间通过页表中的一个**页表项** 映射到一个实际的物理空间页。**Linux 采用的是页式内存管理机制。**    
