...
Linux 的用户进程不能被直接创建出来，因为不存在这样的 API，只能从某个进程中复制出来，再通过 exec 这样的 API 切换到实际想要运行的进程。复制出新的进程的 API 有三种：`fork`, `vfork`, `clone`。这三个 API 内部都是调用一个内和内部函数 `do_fork` ，只是填写的参数不同。              

vfork 是 fork 的部分过程，用以简化并提高效率。而 fork 和 clone 则是有区别的：       
- fork 是进程资源的完全复制，包括 **进程控制块（Processor Control Block, PCB），线程的系统堆栈，进程的用户空间，进程打开的设备** 等。         
- clone 只有前两项 （PCB， 线程系统堆栈） 被复制，后两项 （进程的用户空间，进程打开的设备） 则与父进程共享。             

在思想资源的复制中，用户空间是相对庞大的，如果完全复制则效率会很低。Linux 采用的是 **写时复制** 技术，也就是说，fork 执行时并不真正肤质用户控件的所有页面，而只复制页面表。这样无论父进程还是子进程，当发生用户空间的写操作时，都会引发 **写复制** 操作而另行分配一段可用的用户空间，使其完全独立。这是一种提高效率的非常有效的方法。        
...
怎么从机制上解决以上问题呢？也就是，一方面减少内存碎片的产生，另一方面需要进行内存交换的时候，让需要写入祸从磁盘装载的数据更少一些？答案是内存分页。        
